				...
				formatOptionLabel={ ( data ) => {
					if ( data.svg ) {
						if ( dividers.top.inverted && data['svg-inverted']) {
							return (
								<span className="gutenslider-divider-top" dangerouslySetInnerHTML={ { __html: data['svg-inverted'] } } />
							);
						}
						return (
							<span className="gutenslider-divider-top" dangerouslySetInnerHTML={ { __html: data.svg } } />
						);
					}
					return (
			...
			>
				...
				{ ( animation === 'slide' || animation === 'coverflow' ) &&
					<RangeControl
						...
						help={ slidesToShow === 0 ? __( 'Auto Slides per view will set the slider to the height you set in the "Minimum Slider Height" setting, and disable automatic height recognition. So check yourself that the content does not overflow the slides.', 'gutenslider' ) : null }
						min={ 0 }
						max={ 1 }
						step={ 1 }
						marks={ [
