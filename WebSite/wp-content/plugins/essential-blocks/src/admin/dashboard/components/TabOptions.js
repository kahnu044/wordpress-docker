...
import Facebook from './modal/facebook';
...
export default function TabOptions() {
	...
	const apiIntegrations = applyFilters( 'eb_admin_option_integrations', {
		facebookToken: {
			// Stopgap SVG — design to ship logo-facebook.png matching the other integration logos.
			logo: `${ EssentialBlocksLocalize.image_url }/admin/logo-facebook.svg`,
			title: __( 'Facebook', 'essential-blocks' ),
			description: __(
				'To showcase your Facebook Page feed on your website, add a Page Access Token and Page ID.',
				'essential-blocks'
			),
			doc: 'https://essential-blocks.com/docs/retrieve-facebook-access-token/',
			component: Facebook,
		},
		googleMapApi: {
					...
					{ Object.keys( apiIntegrations ).map( ( item, index ) => (
						...
						>
							...
							<p className="eb-admin-block__text">
								{ __(
									apiIntegrations[ item ].description,
									'essential-blocks'
								) }
							</p>
							<a
