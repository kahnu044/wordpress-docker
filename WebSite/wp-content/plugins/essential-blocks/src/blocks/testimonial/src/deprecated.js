...
const Deprecated = [
    ...
    // Entry #4 — captures the avatar-as-empty-div save (CSS background-image
    // on `.eb-avatar-style`). The latest save now embeds a real <img> inside
    // that div so the morphing runtime can wrap/animate it; existing posts
    // saved against the empty-div markup continue to validate here.
    {
        attributes: { ...attributes },
        supports: {
            align: ["wide", "full"],
        ...
        save: ({ attributes }) => {
            ...
            attributes.className = replaceString(
